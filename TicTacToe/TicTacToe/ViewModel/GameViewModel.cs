...
namespace TicTacToe
{
    public class GameViewModel : ViewModelBase
    {
        ...
        public GameViewModel(Page page) :base(page)
        {
            ...
            CurrentStatus = $"{Settings.Player1} está jugando.";
        }
            ...
            get
            {
                return resetCommand ??
                  (resetCommand = new Command(async () =>
                  {
                      ...
                      if(!GameOver)
                      {
                          var result = await UserDialogs.Instance.ConfirmAsync("Aún no termina el juego, estás seguro de que quieres resetear?", "Resetear?");
                          if (!result)
                              return;
                      }
        ...
        async Task Play(string number)
        {
            ...
            {
                CurrentGame[x, y] = "X";
                CurrentStatus = $"{Settings.Player2} está jugando.";
            }
            else
            {
                CurrentGame[x, y] = "O";
        ...
        async Task CheckResults()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                {
                    GameOver = true;
                    CurrentStatus = $"{Settings.Player1} Ganó!";
                ...
                {
                    GameOver = true;
                    CurrentStatus = $"{Settings.Player2} Ganó!";
            ...
            //Draw!
            GameOver = true;
            CurrentStatus = "Se acabo el juego!";
